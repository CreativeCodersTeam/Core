...
public class XmlRpcValueRequestHandler : XmlRpcRequestHandlerBase
{
    ...
    {
        return this.ExecuteGenericMethod<object>(nameof(GetXmlRpcValueAsync), new[] { requestData.ValueType }, requestData);
    }
    ...
    private async Task<T> GetXmlRpcValueAsync<T>(RequestData requestData)
        where T : XmlRpcValue
    {
        return await GetValueWithExceptionHandlingAsync(() => GetXmlRpcValueInternalAsync<T>(requestData), requestData).ConfigureAwait(false);
    }
